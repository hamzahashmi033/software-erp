...
export function InvoiceTableRow({ invoice, onRefresh }: InvoiceTableRowProps) {
  ...
  return (
    <tr className="border-t border-[#FFEDD5] hover:bg-[#FFF7ED] transition-colors">
      <td className="px-4 py-3">
        <Link
          href={`/invoices/${invoice.id}`}
          className="font-medium text-gray-900 hover:text-[#EA580C] transition-colors"
        >
          {invoice.clientName}
        </Link>
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex items-center justify-end gap-1">
          ...
          <div className="relative inline-block">
            ...
            {menuOpen && (
              <div className="absolute right-0 z-50 mt-1 w-44 rounded-lg border border-[#FED7AA] bg-white py-1 shadow-xl">
                <Link
                  href={`/invoices/${invoice.id}`}
                  className="flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-[#FFF7ED]"
                ...
                {invoice.stripeInvoiceId && invoice.status !== "PAID" && invoice.status !== "VOID" && (
                  <button
                    onClick={handleSync}
                    className="flex w-full items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-[#FFF7ED]"
                  >
                ...
                {invoice.status !== "VOID" && invoice.status !== "PAID" && (
                  <button
                    onClick={handleVoid}
                    className="flex w-full items-center gap-2 px-3 py-2 text-xs text-red-500 hover:bg-[#FFF7ED]"
                  >
